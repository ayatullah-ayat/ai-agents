...
class Agent:
    ...
    def update_memory(self, memory: Memory, response: str, result: dict):
        ...
        new_memories = [
            ...
            {"type": "environment", "content": str(json.dumps(result))}
        ]
...
if __name__ == "__main__":
    ...
    goals = [
        Goal(priority=1,
              ...
              description=(
                    "You are building a README. "
                    "Decide which project file to inspect next. "
                    "CALL EXACTLY ONE TOOL PER RESPONSE. Decide which tool to call based on the current context. Always return at first position the tool call in the response. "
                    "I will respond with the tool result; then you may choose the next tool. "
                )),
        Goal(priority=1,
              name="Terminate",
              description=(
                    "When you have enough information to write a good README, "
                    "call terminate(message=README_MARKDOWN)."
                ))
    ]
